...
if [ ! -z "$POSTGRES_PORT" ] ; then
   ...
   valid_usermgmt_db_port="$?"
   if [ ! "$valid_usermgmt_db_port" -eq "0" ] ; then
      echo "invalid usermgmt db port number"
      exit 1
   fi
fi
...
valid_usermgmtdb_password="$?"
if [ ! "$valid_usermgmtdb_password" -eq "0" ] ; then
   echo "invalid user-mgmt password, complexity validation failed"
   exit 1
fi
